...
@router.get("/", response_model=List[schemas.ExpenseResponse], summary="List all expenses")
def list_expenses(
    skip: int = 0,
    limit: int = 100,
    vehicle_id: str = None,
...
@router.get("/{expense_id}", response_model=schemas.ExpenseResponse, summary="Get an expense")
def get_expense(
    expense_id: int = Path(..., ge=1),
    db: Session = Depends(get_db),
    _: models.User = Depends(get_current_user),
):
    expense = db.query(models.FuelExpense).filter(models.FuelExpense.id == expense_id).first()
    if not expense:
